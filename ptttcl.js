...
exports.callback = async (req, res) => {
  ...
  try {
    ...
    result.return_code = 0; // ZaloPay server sẽ retry callback
...
exports.zaloPay = async (req, res) => {
  ...
  try {
    // Create new order in the database with the provided details
    ...
    // Prepare the payment request to ZaloPay
    const embed_data = {
      redirecturl: "http://localhost:3001",  // Change to the actual redirect URL after payment
    };

    const paymentData = {
      app_id: config.app_id,
      app_trans_id: app_trans_id,
      app_user: orderDetails.id_nguoi_dung,  // Use the actual user ID for the payment
      app_time: Date.now(),
      item: JSON.stringify(orderDetails.chi_tiet_don_hang),
      embed_data: JSON.stringify(embed_data),
      amount: amount,
      description: `Payment for order #${transID}`,
      bank_code: "", // Optionally, specify the bank code
      callback_url: "https://a6a6-171-243-49-49.ngrok-free.app/pttt/callback",  // Callback URL for payment status
    };

    // Generate HMAC for ZaloPay authentication
